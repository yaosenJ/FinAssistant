...
def _detect_receivables_surge(ts_code, report_date, income, balance):
    ...
    receivables = _extract_field(balance, '应收账款', '应收账款及应收票据')
    ...
    if prev_balance:
        prev_recv = _extract_field(prev_balance, '应收账款', '应收账款及应收票据')
...
def _detect_margin_volatility(ts_code, report_date, income):
    ...
    if prev_income:
        ...
        prev_cost = _extract_field(prev_income, '营业成本')
        if prev_rev and prev_cost and prev_rev > 0:
            ...
            if margin_chg < -10:
                alerts.append({
                    ...
                    'detail': f"毛利率: {prev_margin:.1f}% → {curr_margin:.1f}% (下降{abs(margin_chg):.1f}个百分点)",
                    'explanation': '毛利率大幅下降，可能面临成本上升或产品降价压力',
                })
