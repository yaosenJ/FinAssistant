...
def _calc_growth_rates(ts_code, report_date, is_bank):
    ...
    prev_income, prev_date = _get_prev_report(ts_code, 'income', report_date)
    if not prev_income:
        return result
    ...
    if is_bank:
        curr_revenue = _extract_field(curr_income, '净利息收入')
        prev_revenue = _extract_field(prev_income, '净利息收入')
    else:
        curr_revenue = _extract_field(curr_income, '营业总收入', '营业收入')
        prev_revenue = _extract_field(prev_income, '营业总收入', '营业收入')

    curr_np = _extract_field(curr_income, '净利润', '归属于母公司所有者的净利润')
    prev_np = _extract_field(prev_income, '净利润', '归属于母公司所有者的净利润')

    if curr_revenue and prev_revenue and prev_revenue != 0:
        result['营收增长率'] = round((curr_revenue - prev_revenue) / abs(prev_revenue) * 100, 2)

    if curr_np and prev_np and prev_np != 0:
        result['净利润增长率'] = round((curr_np - prev_np) / abs(prev_np) * 100, 2)

    result['_prev_report_date'] = prev_date
...
def get_financial_data(ts_code, report_date=None):
    ...
    Args:
        ...
        report_date: 报告日，如 20260331，None则取最新
...
def calc_fundamental_trend(ts_code, periods=4):
    ...
    Returns:
        dict: {
            ts_code, is_bank,
            trend: [
                {report_date, ROE, 毛利率, 净利率, 资产负债率, 经营现金流净利润比, 营收增长率, 净利润增长率},
                ...
            ]
        }
    """
    ...
    for rd in report_dates[:periods]:
        ...
        display = {
            ...
            '营收增长率': indicators.get('营收增长率'),
            '净利润增长率': indicators.get('净利润增长率'),
        }
