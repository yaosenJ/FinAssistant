...
config = load_config('../congfig/config.yaml')
...
class NewsSearchAnswerSystem:
    ...
    def search_news(self, query: str, days: int = None) -> List[Dict]:
        ...
        return sorted(filtered_results, key=lambda x: x['score'], reverse=True)[:2]
    ...
    def answer(
        ...
        """
        retrieved_texts = self.search_news(question, int(10))
        user_prompt = self.USER_PROMPT.format(
            context="\n".join(str(retrieved_texts)), question=question
        )
        QWEN_API_KEY = "sk-48d14c208910"
...
question_list = df_input["question"].tolist()[800:]
ground_truth_list = df_input["answer"].tolist()[800:]
# print(question_list)
...
# 保存处理结果（仅包含成功处理的数据）
output_path = "../data/qa_pairs/rag_results_2776_v1.csv"
