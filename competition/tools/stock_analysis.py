...
def _rate_fundamental(data):
    ...
    margin = data.get('毛利率') or data.get('营业利润率') or 0
...
def generate_stock_report(ts_code, report_type='brief'):
    ...
    lines = [
        ...
        "一、基本面分析",
        "─" * 50,
        f"  ROE: {_format_value(fundamental.get('ROE'), '%'):<16}  毛利率: {_format_value(fundamental.get('毛利率') or fundamental.get('营业利润率'), '%')}",
    ...
    if report_type == 'detail':
        ...
        if 'trend' in trend and trend['trend']:
            # 银行股用营业利润率，非银行股用毛利率
            margin_key = '营业利润率' if any(t.get('营业利润率') is not None for t in trend.get('trend', [])) else '毛利率'
            lines.extend([
                "",
                "四、基本面趋势（最近4期）",
                "─" * 50,
                f"  {'报告期':<12} {'ROE':<8} {margin_key:<8} {'营收同比':<10} {'净利同比':<10}",
            ])
            for t in trend['trend']:
                lines.append(
                    f"  {t.get('report_date', '-'):<12} "
                    f"{_format_value(t.get('ROE'), '%'):<8} "
                    f"{_format_value(t.get('毛利率') or t.get('营业利润率'), '%'):<8} "
