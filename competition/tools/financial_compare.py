...
def _calc_indicators(income, balance, cashflow):
    ...
    if revenue and cost and revenue > 0:
        # 银行股无营业成本，用营业支出计算营业利润率
        margin_name = '营业利润率' if _extract_field(income, '营业成本') is None else '毛利率'
        result[margin_name] = round((revenue - cost) / revenue * 100, 2)
...
def compare_companies(ts_codes=None, sector_name=None, sector_type='industry', report_date=None, report_type='income'):
    ...
    # 按毛利率/营业利润率排名（银行股用营业利润率，非银行股用毛利率）
    gm_sorted = sorted(companies, key=lambda c: c.get('毛利率') or c.get('营业利润率') or -9999, reverse=True)
    margin_label = '毛利率' if any(c.get('毛利率') is not None for c in companies) else '营业利润率'
    result += f"  {margin_label}排名: " + " > ".join([f"{c['name']}({c.get('毛利率') or c.get('营业利润率', '--')}%)" for c in gm_sorted[:5] if c.get('毛利率') is not None or c.get('营业利润率') is not None]) + "\n"
...
def compare_periods(ts_code, report_type='income', periods=4):
    ...
    # 对比指标（银行股用营业利润率，非银行股用毛利率）
    if report_type == 'income':
        has_margin = any(r.get('毛利率') is not None for r in reports)
        margin_key = '毛利率' if has_margin else '营业利润率'
        metrics = ['营业收入', '净利润', margin_key, '净利率', 'ROE']
