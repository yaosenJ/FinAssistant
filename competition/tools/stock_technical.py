...
def calc_technical_indicators(ts_code, days=120):
    ...
    Returns:
        dict: {
            ...
            pct_5d, pct_10d, pct_20d, vol_ratio, volatility_20d
        }
    """
    ...
    # 动量因子（利用已获取的K线数据计算）
    volumes = [k['volume'] for k in kline if k['volume'] is not None]

    # 近N日收益率 (%)
    pct_5d = round((closes[-1] / closes[-6] - 1) * 100, 2) if len(closes) >= 6 else None
    pct_10d = round((closes[-1] / closes[-11] - 1) * 100, 2) if len(closes) >= 11 else None
    ...
    if len(volumes) >= 20:
        ...
        if avg_20 > 0:
            vol_ratio = round(avg_5 / avg_20, 2)

    # 20日波动率（日收益率标准差 × sqrt(250) 年化）
    volatility_20d = None
    if len(closes) >= 21:
        daily_returns = [(closes[i] / closes[i-1] - 1) for i in range(-20, 0)]
        mean_ret = sum(daily_returns) / len(daily_returns)
        variance = sum((r - mean_ret) ** 2 for r in daily_returns) / len(daily_returns)
        volatility_20d = round((variance ** 0.5) * (250 ** 0.5) * 100, 2)
...
def calc_technical_summary(ts_code, days=120):
    ...
    lines = [
        ...
        "",
        "【动量因子】",
        f"  近5日收益率: {result.get('pct_5d', '--')}%",
        f"  近10日收益率: {result.get('pct_10d', '--')}%",
        f"  近20日收益率: {result.get('pct_20d', '--')}%",
        f"  量比(5/20): {result.get('vol_ratio', '--')}",
        f"  20日波动率(年化): {result.get('volatility_20d', '--')}%",
    ]
