...
def get_constituent_distribution(sector_name, sector_type='industry', trade_date=None):
    ...
    if len(matches) > 1:
        return _format_candidates(matches, sector_name, sector_type)
...
def get_sector_correlation(sector_name1, sector_name2, sector_type='industry'):
    ...
    matches1 = _search_sector(sector_type, sector_name1)
    matches2 = _search_sector(sector_type, sector_name2)

    if not matches1:
        return f"未找到{type_name}板块: {sector_name1}"
    if len(matches1) > 1:
        return _format_candidates(matches1, sector_name1, sector_type)
    if not matches2:
        return f"未找到{type_name}板块: {sector_name2}"
    if len(matches2) > 1:
        return _format_candidates(matches2, sector_name2, sector_type)

    code1, name1 = matches1[0]
    code2, name2 = matches2[0]
